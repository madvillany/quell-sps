jQuery(document).ready(function ($) {
    ...
    function goToByScroll(dataslide) {
        htmlbody.animate({
            scrollTop: $('.page-slide[data-slide="' + dataslide + '"]').offset().top
        }, 2000,'easeInOutQuint');
    }
...
//change top nav style after scrolling down
$(document).ready(function(){       
    var scroll_start = 0;
    var startchange = $('#slide1');
    var endchange = $('#slide5');
    var startoffset = startchange.offset();
    var endoffset = endchange.offset();
    $(document).scroll(function() { 
        scroll_start = $(this).scrollTop();
        if(scroll_start > startoffset.top && scroll_start < (endoffset.top - 150) ) {
            $('#wsrc-navbar').css('border-bottom', '1px solid #B2B5B8');
            $('.navbar-wraper').css('background-color', '#fff');
        } else{
            $('#wsrc-navbar').css('background-color', '#fff');
            $('.navbar-wraper').css('background', 'none');
        }
   });
